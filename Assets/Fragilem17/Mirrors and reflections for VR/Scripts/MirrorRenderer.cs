...
namespace Fragilem17.MirrorsAndPortals
{
	...
	public class MirrorRenderer : MonoBehaviour
	{
		...
		private readonly List<PooledTexture> _pooledTextures = new();

		private static readonly Dictionary<Camera, Camera> _reflectionCameras = new();
		private static readonly Dictionary<Camera, UniversalAdditionalCameraData> _UacPerCameras = new();
		private static readonly Dictionary<Camera, Skybox> _SkyboxPerCameras = new();
		...
		private readonly List<CameraMatrices> _cameraMatricesInOrder = new();
		...
		private Rect _defaultViewRect = new(0, 0, 1, 1);
		...
		private void UpdateCamera(UnityEngine.Rendering.ScriptableRenderContext src, Camera renderCamera)
		{
			...
			else
			{
				...
				_mySkyboxMaterial = _skyboxRenderCam != null ? _skyboxRenderCam.material : RenderSettings.skybox;
			}
			...
			foreach (MirrorSurface reflectionMs in mirrorSurfaces)
			{
					...
					) // && (depth != 2 || (depth == 2 && reflectionMs.VisibleInBoundsParent(parentSurface, _reflectionCamera, renderCamera)))
				{
					...
					if (myDistance <= reflectionMs.maxRenderingDistance || reflectionMs.useRecursiveDarkening)
					{
						...
						Vector4 reflectionPlane = new(mirrorNormal.x, mirrorNormal.y, mirrorNormal.z, d);
						...
						Matrix4x4 newProjectionMatrix = renderCamera.cameraType != CameraType.SceneView && XRSettings.enabled && _UacAllowXRRendering
							? _reflectionCamera.GetStereoProjectionMatrix(eye)
		...
		private void RenderMirrorCamera(UnityEngine.Rendering.ScriptableRenderContext src, Camera reflectionCamera, List<CameraMatrices> cameraMatricesInOrder, Camera.StereoscopicEye eye)
		{
			...
			for (int i = 0; i < cameraMatricesInOrder.Count; i++)
			{
                ...
                if(enableMacOSXTemporaryLogsToAvoidCrashingTheEditor){
					...
					{
						UniversalRenderPipeline.SingleCameraRequest req = new()
						{
...
#pragma warning restore CS0618 // Type or member is obsolete
		...
		private void GetFreeTexture(out PooledTexture textureOut, Camera.StereoscopicEye eye = Camera.StereoscopicEye.Left)
		{
			...
			if (tex == null)
			{
				...
				RenderTextureDescriptor desc = new((int)textureSize.x, (int)textureSize.y, _renderTextureFormat, 1)
				{
					//useMipMap = true,
		...
		private void Update()
		{
				...
				|| _oldUseScreenScaleFactor != useScreenScaleFactor)
			{
				...
				{
					//DestroyImmediate(tex.texture);
					if (tex.texture)
					{
						RenderTexture.ReleaseTemporary(tex.texture);
						tex.texture = null;
					}
				}
		...
		private void GetUACData(Camera renderCamera, out UniversalAdditionalCameraData uac)
		{
			...
			if (!_UacPerCameras.TryGetValue(renderCamera, out UniversalAdditionalCameraData uacOut))
			{
		...
		private void GetCustomSkybox(Camera renderCamera, out Skybox skybox)
		{
			...
			if (!_SkyboxPerCameras.TryGetValue(renderCamera, out Skybox skyboxOut))
			{
		...
		private void CreateMirrorCameras(Camera renderCamera, out Camera reflectionCamera)
		{
			...
			_reflectionCameras.TryGetValue(renderCamera, out Camera reflectionCam);

			if (reflectionCam == null)
			{
				//Debug.Log("new reflection camera for " + renderCamera.name);
				GameObject go = new("Reflection Camera for " + renderCamera.name + " (autoGen, not saved in scene)", typeof(Camera), typeof(Skybox));
				...
				if (!mySkyboxMaterial)
				{
					...
					mySkyboxMaterial = mySky ? mySky.material : RenderSettings.skybox;
				}
		...
		{
			//_ = (mirrorRendererInstances?.Remove(this));
			OnDisable();
		}
		...
		private void OnDisable()
		{
			...
			{
				foreach (var ms in mirrorSurfaces)
				{
					if (ms)
					{
						// Stop sampling any now-dead RTs
						ms.UpdateMaterial(Camera.StereoscopicEye.Left, null, this, 1, Mathf.Infinity,
										  _currentRenderCamera ? _currentRenderCamera.transform.position : Vector3.zero);
						ms.UpdateMaterial(Camera.StereoscopicEye.Right, null, this, 1, Mathf.Infinity,
										  _currentRenderCamera ? _currentRenderCamera.transform.position : Vector3.zero);
					}
				}
			}
			...
			{
				if (kvp.Value) kvp.Value.targetTexture = null;
			}
			...
			{
				foreach (KeyValuePair<Camera, Camera> kvp in _reflectionCameras)
				{
					DestroyImmediate(kvp.Value.gameObject);
				}
				foreach (PooledTexture pTex in _pooledTextures)
				{
					DestroyImmediate(pTex.texture);
				}
			}
			else
			{
				...
				foreach (PooledTexture pTex in _pooledTextures)
				{
					//Destroy(pTex.texture);
					RenderTexture.ReleaseTemporary(pTex.texture); pTex.texture = null;
				}
			}
		...
		private static Matrix4x4 OffAxisProjectionMatrix(float near, float far, Vector3 pa, Vector3 pb, Vector3 pc, Vector3 pe)
		{
			...
			Matrix4x4 p = new(); // projection matrix 
			p[0, 0] = 2.0f * near / (r - l);
			p[0, 1] = 0.0f;
			p[0, 2] = (r + l) / (r - l);
			p[0, 3] = 0.0f;
			...
			Matrix4x4 rm = new(); // rotation matrix;
			rm[0, 0] = vr.x;
			rm[0, 1] = vr.y;
			rm[0, 2] = vr.z;
			rm[0, 3] = 0.0f;
			...
			Matrix4x4 tm = new(); // translation matrix;
			tm[0, 0] = 1.0f;
			tm[0, 1] = 0.0f;
			tm[0, 2] = 0.0f;
			tm[0, 3] = -pe.x;
