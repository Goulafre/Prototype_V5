...
namespace Fragilem17.MirrorsAndPortals
{
    ...
    public class MirrorSurface : MonoBehaviour
    {
		...
		[SerializeField, Range(1, 4)] private int blurPasses = 2;
		private RenderTexture[] blurDown;
		private RenderTexture[] blurUp;
        ...
        */
        ...
        public void UpdateMaterial(Camera.StereoscopicEye eye = Camera.StereoscopicEye.Left, RenderTexture texture = null, MirrorRenderer myRenderer = null, int depth = 1, float distance = 0, Vector3 camPos = default(Vector3))
        {
            if (MyMeshRenderer && _material)
            {
               ...
               //Debug.Log(name + " : " + distance + " : " + depth);
				...
				if (texture != null && kawaseBlurMaterial != null)
				{
					RenderTexture blurredTex = ApplyDualKawaseBlur(texture);

					if (eye == Camera.StereoscopicEye.Left && m.HasProperty("_TexLeft"))
					{
						m.SetTexture("_TexLeft", blurredTex);
					}
					else if (eye == Camera.StereoscopicEye.Right && XRSettings.enabled && m.HasProperty("_TexRight"))
					{
						m.SetTexture("_TexRight", blurredTex);
					}
		...
		private RenderTexture ApplyDualKawaseBlur(RenderTexture source)
		{
			int width = source.width;
			int height = source.height;
			RenderTexture current = source;

			blurDown = new RenderTexture[blurPasses];
			blurUp = new RenderTexture[blurPasses];

			// Downsample passes
			kawaseBlurMaterial.DisableKeyword("_ISUPSAMPLE");
			for (int i = 0; i < blurPasses; i++)
			{
				width = Mathf.Max(1, Mathf.Min(1024, width / 2));
				height = Mathf.Max(1, Mathf.Min(1024, height / 2));
				blurDown[i] = RenderTexture.GetTemporary(width, height, 0, source.format);
				kawaseBlurMaterial.SetFloat("_Offset", 1 + i); // optional: for controlling blur spread
				kawaseBlurMaterial.SetTexture("_MainTex", current);
				Graphics.Blit(current, blurDown[i], kawaseBlurMaterial);
				current = blurDown[i];
			}

			// Upsample passes
			kawaseBlurMaterial.EnableKeyword("_ISUPSAMPLE");
			for (int i = blurPasses - 2; i >= 0; i--)
			{
				blurUp[i] = RenderTexture.GetTemporary(blurDown[i].width, blurDown[i].height, 0, source.format);
				kawaseBlurMaterial.SetFloat("_Offset", 1 + i);
				kawaseBlurMaterial.SetTexture("_MainTex", current);
				Graphics.Blit(current, blurUp[i], kawaseBlurMaterial);
				RenderTexture.ReleaseTemporary(blurDown[i + 1]);
				current = blurUp[i];
			}

			RenderTexture finalBlur = current;

			// Clean up
			for (int i = 0; i < blurPasses - 1; i++)
				RenderTexture.ReleaseTemporary(blurDown[i]);
			for (int i = 0; i < blurPasses - 1; i++)
				if (blurUp[i] != finalBlur) RenderTexture.ReleaseTemporary(blurUp[i]);
